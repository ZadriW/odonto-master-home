...
function updateShippingSummary() {
    ...
    if (confirmBtn) {
        ...
        if (selectedCount < totalGroups) {
            confirmBtn.title = `Selecione o frete para todos os ${totalGroups} centros de distribuição`;
        } else {
...
async function confirmShippingQuoteGroups() {
    ...
    try {
        ...
        // Verificar se é Multi-CD (mais de uma seleção ou tem distributionCenterId)
        const isMultiCD = shippingSelections.length > 1 || 
                         (shippingSelections.length === 1 && shippingSelections[0].distributionCenterId);
        
        // Method 1: Try the SDK selectShippingQuote method if available
        if (typeof client.checkout?.selectShippingQuote === 'function') {
            // Para Multi-CD, chamar para cada CD
            for (const selection of shippingSelections) {
                ...
                // Adicionar distributionCenterId se for Multi-CD
                if (selection.distributionCenterId) {
                    params.distributionCenterId = selection.distributionCenterId;
                }
                
                result = await client.checkout.selectShippingQuote(params);
            }
        }
        // Method 2: Use GraphQL mutation directly (recomendado para Multi-CD)
        else {
        ...
        if (confirmBtn) {
            ...
            confirmBtn.innerHTML = 'Confirmar Frete';
        }
    }
}
...
async function updateCheckoutAfterShippingSelection(checkoutId) {
    console.log('Shipping selected successfully, reloading page to update checkout...');
    
    // Mostrar mensagem de sucesso e recarregar após um breve delay
    // Isso permite que o usuário veja a confirmação antes do reload
    setTimeout(() => {
    ...
    try {
        ...
        for (const selection of shippingSelections) {
            ...
            if (isMultiCD && selection.distributionCenterId) {
                // Multi-CD: incluir distributionCenterId
                ...
                console.log('Selecting shipping (multi-CD) for CD:', selection.distributionCenterId, 'Quote:', selection.shippingQuoteId);
            } else {
                // CD único: mutation simples
                mutation = simpleMutation;
                variables = {
                    checkoutId: checkoutId,
                    shippingQuoteId: selection.shippingQuoteId
                };
                console.log('Selecting shipping (single CD) Quote:', selection.shippingQuoteId);
            }
            ...
            // Verificar se a mutation foi bem-sucedida
            if (response && response.checkoutSelectShippingQuote) {
                console.log('Shipping selected successfully:', response.checkoutSelectShippingQuote);
            }
        }
